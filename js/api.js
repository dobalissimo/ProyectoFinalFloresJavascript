document.addEventListener('DOMContentLoaded', function() {
  ...
  const pokeballPrice = 200; // Price of each Pokéball
  let pokeballsLeft = tries; // Initialize the number of pokeballs left
  ...
  catchButton.addEventListener('click', function() {
      ...
      if (pokeballsLeft > 0) {
          ...
          showMessage(`You ran out of Pokéballs. Visit the nearest mart to get more for $${pokeballPrice} each.`);
      }
  });
  ...
  function updatePokeballsLeftText() {
      pokeballsLeftText.textContent = `Pokéballs Left: ${pokeballsLeft}`;
  }
  ...
  function displayTeamMembers() {
    ...
    if (team.length === 0) {
        return; // Return early if the team is empty
    }
